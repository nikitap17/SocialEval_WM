...
mean_data <- new_df %>%
  ...
  summarise(mean_x = mean(diff_err),
            mean_y = mean(diff_rt),
            se_x = sd(diff_err) / sqrt(n()),
            se_y = sd(diff_rt) / sqrt(n()))

reg_col <- "#453947" #"#46396E"
# Create the plot
p <- ggplot(data = new_df, aes(x=diff_err, y=diff_rt)) +
  geom_point(shape = 16, aes(color= CL), size=3, alpha=0.8) +  # scatter plot
  ...
  labs(x="Delta errors", y="Delta rt", title="all CL") +
...
lmm_df$improvement <- factor(lmm_df$improvement)
levels(lmm_df$improvement)

lmm <- lmer(scale(diff_rt) ~ scale(diff_err) + CL + (1|ID), data = lmm_df, REML = T)
summary(lmm)
performance::r2(lmm)
```

```{r}
lmm <- lm(diff_rt ~ improvement + CL, data = new_df)
emm <- estimate_means(lmm, ~ improvement + CL)

plot_emm <- ggplot(emm, aes(x = improvement, y = Mean, group = factor(CL), color=factor(CL),shape=CL, linetype = factor(CL))) +
  geom_point(position = position_dodge(width = 0.2), size = 2.5) +
  geom_line(position = position_dodge(width = 0.2), linewidth = 0.5) +
  geom_errorbar(aes(ymin=CI_low, ymax=CI_high), width=.2,
                position=position_dodge(0.2),linewidth=0.35,linetype=1)+
  ...
  scale_shape_manual(values = c(15, 19),
                     labels = c("low", "high")) +  # Use square for H0 and circle for H1
  scale_color_manual(values = c("#FF8811FF", "#046E8FFF"),
                     labels = c("low", "high")) +  # Orange color for H0 and dark green for H1
  scale_linetype_manual(values = c(1,2),
                       labels = c("low", "high")) +
...
paste("N of participants who performed better in both low and high CL:", sum(incl_list2back %in% incl_list3back))
...
```{r}
tab <- table(experiment_data$dominant_response,experiment_data$Evaluation)
