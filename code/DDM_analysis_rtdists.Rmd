...
```{r}
m1 <- lmer(a ~ CL*Eval + (1|ID), data = main_df, REML = T)
m2 <- lmer(a ~ CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m1)-AIC(m2)
BIC(m1)-BIC(m2)

m3 <- lmer(a ~ z + v1 + v2 + CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m3)-AIC(m2)
BIC(m3)-BIC(m2)
...
emm_mod <- lmer(a ~ CL*Eval + (1|ID), data = main_df, REML = T)
...
```{r}
m1 <- lmer(z ~ CL*Eval + (1|ID), data = main_df, REML = T)
m2 <- lmer(z ~ CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m1)-AIC(m2)
BIC(m1)-BIC(m2)

m3 <- lmer(z ~ a + v1 + v2 + CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m3)-AIC(m2)
BIC(m3)-BIC(m2)
...
```{r}
table <- model_parameters(lmer(scale(z) ~ CL*Eval + (1|ID), data = main_df, REML = T))[1:3,] %>% dplyr::select(c("Parameter","Coefficient", "SE", "CI_low", "CI_high", "p"))
...
```{r}
emm_mod <- lmer(z ~ CL*Eval + (1|ID), data = main_df, REML = T)
emm <- estimate_means(emm_mod, ~CL*Eval)
...
```{r}
m1 <- lmer(v1 ~ CL*Eval + (1|ID), data = main_df, REML = T)
m2 <- lmer(v1 ~ CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m1)-AIC(m2)
BIC(m1)-BIC(m2)

m3 <- lmer(v1 ~ a + z + v2 + CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m3)-AIC(m2)
BIC(m3)-BIC(m2)
...
```{r}
table <- model_parameters(lmer(scale(v1) ~ a + z + v2 + CL*Eval + (1|ID), data = main_df, REML = T)) %>% filter(Parameter %in% c("(Intercept)", "CLhigh", "Eval1")) %>% dplyr::select(c("Parameter","Coefficient", "SE", "CI_low", "CI_high", "p"))
...
```{r}
emm_mod <- lmer(v1 ~ a + z + v2 + CL*Eval + (1|ID), data = main_df, REML = T)
emm <- estimate_means(emm_mod, ~CL*Eval)
...
```{r}
m1 <- lmer(v2 ~ CL*Eval + (1|ID), data = main_df, REML = T)
m2 <- lmer(v2 ~ CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m1)-AIC(m2)
BIC(m1)-BIC(m2)

m3 <- lmer(v2 ~ a + z + v1 + CL+Eval + (1|ID), data = main_df, REML = T)
AIC(m3)-AIC(m2)
BIC(m3)-BIC(m2)
...
```{r}
table <- model_parameters(lmer(scale(v2) ~ a + z + v1 + CL*Eval + (1|ID), data = main_df, REML = T)) %>% filter(Parameter %in% c("(Intercept)", "CLhigh", "Eval1")) %>% dplyr::select(c("Parameter","Coefficient", "SE", "CI_low", "CI_high", "p"))
...
```{r}
emm_mod <- lmer(v2 ~ a + z + v1 + CL*Eval + (1|ID), data = main_df, REML = T)
emm <- estimate_means(emm_mod, ~CL*Eval)
